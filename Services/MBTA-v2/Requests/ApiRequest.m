...
// parse request URL string into key for naming cached JSON files
//	so for request = "http://<ip>/developer/api/v2/<verb>?apiKey=<key>&<p1>=<v1>...&<pN>=<vN>&format=<format>"
...
// else this logic may fail for some requests as order of parameters may vary based on which parameters are included
...
+ (NSString *)keyForRequest:(NSString *)request {
	...
	if ([request length]) {
		...
		if (
			...
			NSException *exception = [NSException exceptionWithName:@"ApiRequestInvalidKeyException" reason:@"ApiRequest can't get key from request URL." userInfo:nil];
			@throw exception;
		}
#endif
...
+ (id)cachedJSONForKey:(NSString *)key staleAge:(double)staleAge {
	...
	if ([key length] && staleAge > 0.0) {
		NSString *path = [AppDelegate fileForKey:key];
		if ([path length]) {
