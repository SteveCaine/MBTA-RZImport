...
- (void)get_servertime {
	...
	[ApiTime get_success:^(ApiTime *servertime) {
		...
		NSLog(@"\n\n%s %@", __FUNCTION__, [error localizedDescription]);
		@strongify(self)
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section {
	...
	switch (section) {
		case 0:
			result = @"get data (no caching)";
			break;
		case 1:
			result = @"request data (with caching)";
			break;
		default:
			break;
	}
		...
		switch (indexPath.row) {
			...
			case e_verb_stopsbylocation:
				if (self.stopsByLocation == nil) {
					...
					// but see comment above on this method
